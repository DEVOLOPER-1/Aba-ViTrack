    ...
    def track(self, image, info: dict = None):
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                cv2.rectangle(image_BGR, (int(x2),int(y2)), (int(x2+w2),int(y2+h2)), color=(0,255,0), thickness=3)
                save_path = os.path.join('{{PROJECT PATH}}/output/debug', "%04d.jpg" % self.frame_id)
