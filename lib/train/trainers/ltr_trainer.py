...
class LTRTrainer(BaseTrainer):
    ...
    def _bbox_to_mask(self, bbox, h, w):
        mask = torch.zeros([h,w])
        # h, w = self.shape
        # x1, y1, x2, y2 = map(int, bbox)
        x1, y1, x2, y2 = map(int,torch.round(bbox))
    ...
    def get_masks(self, template_anno, H, W):
        # template_images=data['template_images']
        # im=template_images[0,0].permute(1,2,0)
        # template_anno=data['template_anno']
        # H=W=template_images.shape[-1]

        template_masks=[]
        for i in range(template_anno.shape[1]):
            bbox=template_anno[0,i]*H
            template_mask=self._bbox_to_mask(box_xywh_to_xyxy(bbox), H, W)
            template_masks.append(template_mask.unsqueeze(0))
        template_masks=torch.cat(template_masks,0)
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            #===========test=====
            template_anno=data['template_anno']
            H=W=data['template_images'].shape[-1]
            t_masks = self.get_masks(template_anno, H, W)

            search_anno=data['search_anno']
            H=W=data['search_images'].shape[-1]
            s_masks = self.get_masks(search_anno, H, W)

            data['t_masks'] = t_masks.to(self.device)
            data['s_masks'] = s_masks.to(self.device)
            ...
            if loader.training:
                for j, _ in enumerate(self.optimizer):
                    self.optimizer[j].zero_grad()
                    if not self.use_amp:
                        loss[j].backward(retain_graph=True)
                        if self.settings.grad_clip_norm > 0:
                            if j == 0:
                                torch.nn.utils.clip_grad_norm_(self.actor.net.parameters(),
                                                               self.settings.grad_clip_norm)
                            else:
                                torch.nn.utils.clip_grad_norm_(self.actor.net2.parameters(),
                                                               self.settings.grad_clip_norm)
                        self.optimizer[j].step()
                    else:
                        self.scaler.scale(loss[j]).backward()
                        if self.settings.grad_clip_norm > 0:
                            self.scaler.unscale_(self.optimizer[j])
                            if j == 0:
                                torch.nn.utils.clip_grad_norm_(self.actor.net.parameters(),
                                                               self.settings.grad_clip_norm)
                            else:
                                torch.nn.utils.clip_grad_norm_(self.actor.net2.parameters(),
                                                               self.settings.grad_clip_norm)
                        self.scaler.step(self.optimizer[j])
                        self.scaler.update()
    ...
    def _update_stats(self, new_stats: OrderedDict, batch_size, loader):
        ...
        # add lr state
        if loader.training:
            for j, _ in enumerate(self.lr_scheduler):
                lr_list = self.lr_scheduler[j].get_last_lr()
    ...
    def _stats_new_epoch(self):
        ...
        for loader in self.loaders:
            if loader.training:
                try:
                    lr_list = self.lr_scheduler[0].get_last_lr()
                except:
                    lr_list = self.lr_scheduler[0]._get_lr(self.epoch)
                for i, lr in enumerate(lr_list):
                    var_name = 'LearningRate/group{}'.format(i)
                    if var_name not in self.stats[loader.name].keys():
                        self.stats[loader.name][var_name] = StatValue()
