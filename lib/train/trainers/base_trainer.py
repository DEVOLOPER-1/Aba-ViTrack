...
class BaseTrainer:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if self.lr_scheduler is not None:
                        if self.settings.scheduler_type != 'cosine':
                            for i, _ in enumerate(self.lr_scheduler):
                    ...
                    if save_every_epoch:
                        if epoch in save_epochs:
                            if self._checkpoint_dir:
                                if self.settings.local_rank in [-1, 0]:
                                    self.save_checkpoint()
                    if epoch % getattr(self.settings, "VAL_EPOCH_INTERVAL", 1) == 0:                    # if epoch > (max_epochs - 10) or save_every_epoch or epoch % 100 == 0:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                self.save_checkpoint()
            except:
    ...
    def save_checkpoint(self):
        ...
        net_num = len(self.optimizer)
        for i, _ in enumerate(self.optimizer):
            if i == 0:
                net = self.actor.net.module if multigpu.is_multi_gpu(self.actor.net) else self.actor.net
            else:
                net = self.actor.net2.module if multigpu.is_multi_gpu(self.actor.net2) else self.actor.net2
            ...
            if net_num == 1:
                ...
                tmp_file_path = '{}/{}_net{}_ep{:04d}.tmp'.format(directory, net_type, i + 1, self.epoch)
                torch.save(state, tmp_file_path)

                file_path = '{}/{}_net{}_ep{:04d}.pth.tar'.format(directory, net_type, i + 1, self.epoch)
